#!/bin/bash
# Short -> long transition curriculum. The preceding long-hop curriculum
# learned isolated first hops but averaged only one touchdown per episode.
# Hold geometry at the learned boundary and train the short landing/stance as
# the preparation state for the following long hop.
set -euo pipefail
...
"$ISAAC_SIM_ROOT/python.sh" "$SCRIPT_DIR/train_planner_circular.py" \
  ...
  --long_curriculum_radius_max 0.435 \
  --long_curriculum_turn_deg 67 \
  --distance_curriculum_iterations 100 \
