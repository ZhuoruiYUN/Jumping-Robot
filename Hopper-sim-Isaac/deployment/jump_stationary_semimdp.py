...
# 2026-09-02 实飞对比：v36 teacher 单跳 apex 1.18 m、空中倾斜 29°、落点偏 0.4 m；
# 用户反馈 two_hop 脚本用的 air050 导出（v60 random_drop_safety model_79）跳得明显更好。
# 2026-09-02 下午：在 air050 基础上加入推力随机化 sim2real 微调，0-0.2/0.2-0.4 m
# visual 无实机式逐跳发散震荡；direct eval death 0/256，apex 0.992 m，落地姿态 0.0597 rad。
# 旧 v36 teacher: 'deployment/stationary_semimdp_export/quadhopper_v22_policy.onnx'
# 旧 air050 teacher: 'deployment/two_hop_release_export_air050/quadhopper_v22_policy.onnx'
TEACHER_POLICY_PATH = 'deployment/two_hop_release_export_sim2real_020_040_m79/quadhopper_v22_policy.onnx'
PLANNER_POLICY_PATH = 'deployment/stationary_semimdp_export/stationary_semimdp_policy_69d.onnx'
# 2026-09-02 实飞：69-D 修正器锁存的 7.9° 前倾指令在实机（推力更强）上放大成
# 28° 离地姿态和 1.2 m/s 侧飞，直接翻车。先摘掉修正器做纯 43-D teacher 对照飞行，
# 确认 teacher 单独在实机上是否稳定；问题锁定后再考虑重标定修正器。
...
# 原地跳部署目标：按下 H 后锁定当前位置作为 landing waypoint。
TARGET_Z = 1.0
...
# 2026-09-02 实飞总结：0.85 → 4 跳发散，0.75 → 6 跳发散；差动单独缩放 0.6 反而更差
# （2 跳 + yaw 甩尾 ±35°/s 加剧——破坏了策略内部 tilt/yaw/高度控制的协调关系）。
# 结论：部署侧标量旋钮无法重调 recurrent 网络的内部增益，模型必须用实测推力曲线
# 做 sim2real 微调。当前保留 0.75 全量作为 6 跳的临时上限。
DEPLOY_ACTION_SCALE = 0.75
            ...
            if not is_calibrating and vicon.new_frame_event.wait(timeout=0.001):
                ...
                if flight_mode in ('CALIBRATE', 'IDLE'):
                    ...
                    if vicon_tick % 2 == 0:
                        ...
                        action_history.append(np.zeros(4, dtype=np.float32) if hold_zero_this_tick else action)
