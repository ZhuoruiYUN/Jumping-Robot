...
# 当前部署模型：m79 far（2026-09-05 物理修正后重新验证：0/0 契约 hit 0.798、td_err 0.085；
# far 契约 0.578/0.450/0.295 @ tm 0.125/0.06/0.03）。模型权重不依赖物理，ONNX 无需重导出。
# 43-D recurrent teacher-only；69-D planner 代码保留但 USE_SEMIMDP_PLANNER=False 不执行。
TEACHER_POLICY_PATH = 'deployment/two_hop_release_export_far_030_060_m79/quadhopper_v22_policy.onnx'
PLANNER_POLICY_PATH = ''
...
# 同点两跳部署目标：按下 H 后锁定当前位置作为 landing waypoint，直接释放开始跳。
TARGET_Z = 1.0
...
# 2026-09-05 实飞：0.30 阈值对 v2_240 实际精度偏紧——0.325m/姿态 3.6° 的稳定落地
# 被误断油。放宽到 0.45：仍远低于发散落地的 0.87-1.08m，能拦住真翻车。
STATIONARY_XY_ABORT_M = 0.45
XY_ABORT_MIN_ACTIVE_S = 0.20
# 落地姿态保护：触地瞬间倾斜超过该角度即断油回 IDLE（自 stationary 脚本移植）。
TOUCHDOWN_TILT_ABORT_DEG = 15.0

# Deployment-side taming. These do not change the ONNX policy, only the real motor command.
# 2026-09-03 实机工作点（tm~0.03）扫描：0.80 最优（hit 0.465 vs 0.85 的 0.390，
# td_err 0.122 vs 0.136）。快电机放大增益，压一点缩放抵消横向修正过冲。
DEPLOY_ACTION_SCALE = 0.80
...
FREE_FALL_TRIGGER_TICKS = 10  # 主循环 200 Hz，10 tick ≈ 50 ms 持续下坠判定

MAX_PWM = 1000
MIN_PWM = 0
# 低压保护：推力随电压平方下降，3.9V 以下拒绝武装、3.65V 以下飞行中强制回 IDLE。
# 2026-09-05 实飞教训：3.61V 起跳推力只剩 ~74%，空中姿态拉不住直接翻车。
MIN_ARM_VBAT_V = 3.90
MIN_FLIGHT_VBAT_V = 3.65
...
def run_autonomous_flight():
    try:
        teacher_policy = StatefulOnnxPolicy(TEACHER_POLICY_PATH, expected_obs_dim=43)
        planner_policy = (
            StatefulOnnxPolicy(PLANNER_POLICY_PATH, expected_obs_dim=69)
            if USE_SEMIMDP_PLANNER
            else None
        )
        print(f"✅ 成功加载 43D two-hop 追点模型: {TEACHER_POLICY_PATH}")
        if USE_SEMIMDP_PLANNER:
            print(f"✅ 成功加载 stationary Semi-MDP 模型: {PLANNER_POLICY_PATH}")
        else:
            print("✅ 当前为一体模型直接部署：不启用悬停/预稳定/SEMIMDP landing correction")
    except Exception as e:
        print(f"❌ 模型加载失败: {e}")
        return
    ...
    def reset_policy_state():
        ...
        nonlocal startup_ready_ticks, previous_pwm_cmd, flight_start_time
        nonlocal soft_tilt_bad_ticks, hard_tilt_bad_ticks
    ...
    try:
        ...
        print(f"🚀 跳跃机系统就绪: 等待传感器校准后，按下 'H' 键锁定当前位置并释放跳跃...")
        ...
        while True:
                ...
                if key in (b'\x00', b'\xe0'):
                    ...
                    if key_lower == b'e':
                        ...
                        if flight_mode in ('IDLE', 'CALIBRATE'):
                            if latest_vbat < MIN_ARM_VBAT_V:
                                print(
                                    f"!!!! 拒绝武装：VBat={latest_vbat:.2f}V < "
                                    f"{MIN_ARM_VBAT_V:.2f}V，请更换/充电电池 !!!!"
                                )
                            else:
                                flight_mode = 'ARMED'
                                ...
                                reset_policy_state()
                                freefall_ticks = 0
                                print("==== ARMED: 已锁定当前位置，电机零输出；检测到松手下坠后自动起跳；再按 H 取消 ====")
                        else:
                            flight_mode = 'IDLE'
                            reset_policy_state()
                            freefall_ticks = 0
                            print("==== IDLE: 已停止输出，按 H 重新武装 ====")
                    elif key_lower == b'k':
            ...
            if not is_calibrating and vicon.new_frame_event.wait(timeout=0.001):
                ...
                if latest_fusion_sample is None or imu_age_s > IMU_STALE_TIMEOUT_S:
                ...
                if flight_mode == 'ARMED':
                    ...
                    if policy_vel_w[2] < FREE_FALL_TRIGGER_VZ:
                        freefall_ticks += 1
                    else:
                        freefall_ticks = 0
                    if freefall_ticks >= FREE_FALL_TRIGGER_TICKS:
                        ...
                        current_target_x, current_target_y = waypoint_queue.targets_xy[0]
                        reset_policy_state()
                        reset_planner_handoff_state(False)
                        flight_start_time = time.perf_counter()
                        hold_zero_this_tick = True
                        print("==== 检测到松手下坠：锁定当前位置，two-hop 模型接管 ====")
                ...
                if flight_mode == 'JUMP' and latest_vbat < MIN_FLIGHT_VBAT_V:
                    m1, m2, m3, m4 = 0, 0, 0, 0
                    previous_pwm_cmd = None
                    deploy_safety_code = 3
                    flight_mode = 'IDLE'
                    reset_policy_state()
                    print(f"!!!! VBAT ABORT: {latest_vbat:.2f}V < {MIN_FLIGHT_VBAT_V:.2f}V，断油回 IDLE !!!!")

                if flight_mode in ('CALIBRATE', 'IDLE', 'ARMED') or hold_zero_this_tick:
                    m1, m2, m3, m4 = 0, 0, 0, 0
                    previous_pwm_cmd = None
                    ...
                    if vicon_tick % 2 == 0:
                        ...
                        p_t_error_b = yaw_world_to_body_xy(p_t - pos_w[:2], latest_pure_q)
                        p_t1_error_b = yaw_world_to_body_xy(p_t1 - pos_w[:2], latest_pure_q)
                        height_commands = np.array([TARGET_Z / 2.0, TARGET_Z / 2.0], dtype=np.float32)
                        ...
                        if flight_mode == 'JUMP':
                            ...
                            if just_touched_down:
                                ...
                                needs_new_plan = True
                                touchdown_count += 1
                                # 防炸机（自 stationary 脚本移植）：落地误差 >0.30 m 或
                                # 落地姿态 >15° 即断油回 IDLE（机器人已在地面，断油安全）。
                        ...
                        last_obs_log = planner_obs.tolist()
                if flight_mode in ('CALIBRATE', 'IDLE', 'ARMED') or not link_health_ok:
                    deploy_pwm_spread = 0.0
    ...
    if log_data:
        ...
        planner_extra_headers = [
            'Obs_Teacher_PtErr_bx', 'Obs_Teacher_PtErr_by',
            'Obs_Teacher_Pt1Err_bx', 'Obs_Teacher_Pt1Err_by',
            'Obs_Teacher_Height_t', 'Obs_Teacher_Height_t1',
            'Obs_TeacherAction_F1', 'Obs_TeacherAction_F2', 'Obs_TeacherAction_F3', 'Obs_TeacherAction_F4',
            'Obs_Phase_Contact', 'Obs_Phase_NoContactInactive', 'Obs_Phase_Ascent', 'Obs_Phase_Descent',
            'Obs_ProjectedErr_bx', 'Obs_ProjectedErr_by', 'Obs_TTL_Norm',
            'Obs_Vxy_bx', 'Obs_Vxy_by', 'Obs_NextDir_bx', 'Obs_NextDir_by',
            'Obs_BodyZAxis_bx', 'Obs_BodyZAxis_by',
            'Obs_SpringPos', 'Obs_SpringVel',
            'Obs_LatchedCmd_Forward', 'Obs_LatchedCmd_Lateral',
            'Obs_LatchedCmd_TiltForward', 'Obs_LatchedCmd_TiltLateral',
            'Obs_Pair_Short', 'Obs_Pair_Long', 'Obs_Pair_FirstHit',
        ]
        obs_headers = teacher_headers + history_headers + planner_extra_headers
        ...
        headers = [
                      ...
                      'Deploy_Tilt_Deg', 'Deploy_XY_Error_M', 'Deploy_PWM_Spread', 'Deploy_Safety_Code',
                      'Flight_Mode'
                  ]
